...
public static class DatabaseSeeder
{
    ...
    {
        Console.WriteLine("[Seeder] Running DatabaseSeeder.Seed...");
        context.Database.Migrate();

        if (!context.Teams.Any())
        {
            Console.WriteLine("[Seeder] No teams found, seeding teams and agents...");
            var officeEndHour = Environment.GetEnvironmentVariable("OFFICE_END_HOUR") ?? "17:00:00";
            var shiftEnd = TimeSpan.Parse(officeEndHour);
            ...
            context.SaveChanges();
            Console.WriteLine("[Seeder] Teams and agents seeded successfully. ShiftEnd used: " + shiftEnd);
        }
        else
        {
            Console.WriteLine($"[Seeder] Teams already exist in database: {context.Teams.Count()} teams.");
        }
    }
}
