...
public class ChatAssignmentService : IChatAssignmentService
{
    ...
    public async Task AssignChatToAgentAsync(Guid chatSessionId)
    {
        ...
        {
            _logger.LogInformation("Assigning chat session {ChatSessionId} to agent {AgentId} (Team {TeamId}, Seniority {Seniority}, CurrentChats {CurrentChats}/{Capacity})",
                chatSessionId, availableAgent.Id, availableAgent.TeamId, availableAgent.Seniority, availableAgent.CurrentChats, availableAgent.Capacity);
